...
<div style="background:#fff;border:1px solid ${G};border-left:3px solid ${G};border-radius:4px;padding:7px 10px;font-size:10.5px;color:#444;line-height:1.55;"><span style="color:${G};font-weight:700;margin-right:4px;">💬</span>두 영역 모두 <b style="color:${N};">${summary}</b> 수준이며, 꾸준한 학습 습관이 잘 형성되어 있습니다.</div>
  ...
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><title>와튼_${d.name}_${d.month}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'Malgun Gothic','Apple SD Gothic Neo',Arial,sans-serif;background:#fff;-webkit-print-color-adjust:exact;print-color-adjust:exact;color-adjust:exact;}table{border-collapse:collapse;width:100%;}@media print{@page{margin:8mm;size:A4;}body{padding:0;}}</style></head><body><div style="max-width:780px;margin:0 auto;"><div style="background:${LBG};padding:14px 28px 12px;text-align:center;"><div style="font-size:14px;letter-spacing:5px;color:${LG};font-weight:700;">WHARTON ENGLISH SCHOOL</div><div style="font-size:11px;letter-spacing:4px;color:#a0925a;margin-top:3px;">MONTHLY PROGRESS REPORT</div></div><table><tr><td style="background:linear-gradient(180deg,${N},#1a3060);width:32px;text-align:center;vertical-align:middle;"><div style="writing-mode:vertical-rl;transform:rotate(180deg);font-size:9px;letter-spacing:4px;color:${G};font-weight:700;padding:14px 0;">REPORT CARD</div></td><td style="padding:12px 18px;vertical-align:top;"><table style="border:2px solid ${N};border-radius:6px;overflow:hidden;margin-bottom:10px;"><tr style="background:linear-gradient(135deg,${N},#1a3060);">${[["Name",d.name],["Class",d.cls],["Teacher",d.tchr],["Month",d.month]].map(([k,v]) => `<td style="padding:7px 12px;border-right:1px solid #2a4070;"><div style="font-size:8px;color:${G};letter-spacing:1px;">${k}</div><div style="font-size:12px;font-weight:700;color:#fff;">${v}</div></td>`).join("")}</tr></table>${cu}${SH("학습진도평가","📚")}<table style="border:1px solid #ddd;border-top:none;margin-bottom:10px;border-radius:0 0 6px 6px;overflow:hidden;">${cr}</table>${SH("학습 분석 리포트","🔍")}<table style="border:1px solid #ddd;border-top:none;margin-bottom:10px;border-radius:0 0 6px 6px;overflow:hidden;">${ar}</table>${achievementBlock}<div style="background:linear-gradient(135deg,${N},#1a3060);padding:8px 14px;border-radius:6px 6px 0 0;"><span style="font-size:9px;letter-spacing:3px;color:${G};font-weight:700;">✍️ TEACHER'S COMMENTS AND FEEDBACK</span></div><div style="border:1px solid #ddd;border-top:none;padding:14px 16px;margin-bottom:10px;border-radius:0 0 6px 6px;background:#fffef8;"><div style="font-size:11.5px;line-height:2.0;color:#222;white-space:pre-line;">${d.cmt}</div></div><div style="border-top:1px solid #eee;padding-top:8px;display:flex;justify-content:space-between;align-items:center;"><div style="background:${LBG};padding:4px 10px;border-radius:6px;"><span style="font-size:9px;color:${LG};font-weight:700;">와튼영어스쿨</span></div><span style="font-size:8px;color:#ccc;">WHARTON ENGLISH SCHOOL — MONTHLY PROGRESS REPORT</span></div>${ps}</td></tr></table></div><script>window.addEventListener("load",function(){setTimeout(function(){window.print();},700);});<\/script></body></html>`;
}
